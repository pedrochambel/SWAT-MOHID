     ...
     *          sol_nh3(ly,j)
	    ...
	    !! write daily and/or monthly output
          ...
          if (SwatWithChanges) then
	        ...
	        OutputGlobalBasin = wshddayo

              call ModifySwat(ObjSwatID=MainObjSwatID, 
     &                        OutputHruMeteo=OutputHruMeteo,
     &                        OutputHruQuality=OutputHruQuality,
     &                        OutputHruWatBal=OutputHruWatBal,
     &                        OutputHruParameters=OutputHruParameters,
     &                        OutputSubBasin=OutputSubBasin,
     &                        OutputSubBasin2=OutputSubBasin2,
     &	                    OutputReaches=OutputReaches,
     &                        OutputGlobalBasin=OutputGlobalBasin,
     &						OutputHRU_Cropname=OutputHRU_Cropname,
     &						HRU_output_values=HRU_output_values,
     &                        iyr=iyr, ida=ida)                                         
	        OutputHruQuality = 0.0
              OutputHruMeteo   = 0.0
	    endif
