...
!! summarize output/determine loadings to next routing unit
      ...
      if (SwatWithChanges) then
        !Out flow of the channel
        OutputSubBasin(5,inum1) =   
     &        rtwtr / 86400.

	  OutputReaches(1,inum1) = rtwtr / 86400.

        OutputReaches(2,inum1) = algae(inum1)   
        OutputReaches(3,inum1) = ammonian(inum1)
        OutputReaches(4,inum1) = rch_cbod(inum1)     
        OutputReaches(5,inum1) = chlora(inum1)  
        OutputReaches(6,inum1) = disolvp(inum1) 
        OutputReaches(7,inum1) = rch_dox(inum1)   !PCL   
        OutputReaches(8,inum1) = nitraten(inum1)
        OutputReaches(9,inum1) = nitriten(inum1)
        OutputReaches(10,inum1)= organicn(inum1)
        OutputReaches(11,inum1)= organicp(inum1)
        OutputReaches(12,inum1)= soxy       
        OutputReaches(14,inum1)= sedrch       
	endif
      return
      end
